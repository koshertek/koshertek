...
echo.
echo [31mFor a reflash enter ref with no numbers.[0m 
echo.
...
REM echo [31mPLEASE NOTE: The installation for this model TCL is different than others.[0m
REM echo [31mThe phone will enter a few different modes. DO NOT touch the phone until the tool says it is done!![0m
REM echo [31mDO NOT press[0m[32m "REBOOT SYSTEM" [0m[31mon the phone.[0m
REM echo.
...
"%koshertek%\adb\fastboot.exe" --disable-verity --disable-verification flash pgpt "%koshertek%\4058R\pgpt" >nul 2>&1
TASKKILL /F /IM autobooter.exe >nul 2>&1
start /B "" "%koshertek%\adb\autobooter.exe" >nul 2>&1
:atbtr:
tasklist | findstr /i autobooter.exe >nul 2>&1
if errorlevel 1 (
